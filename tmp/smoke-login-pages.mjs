...
if (!EMAIL || !PASSWORD) {
  console.error('Missing LOGIN_EMAIL or LOGIN_PASSWORD');
  process.exit(2);
}
...
(async () => {
  ...
  try {
    ...
    if (createdDocId) {
      ...
      add('Open /preview/[id]', status === 200 && !hasNotFound, `status=${status} notFound=${hasNotFound}`);
    }
  ...
  console.log('\nSUMMARY');
  console.log(JSON.stringify({ base: BASE, createdDocId, total: results.length, passed, failed, results, finalUrl: page.url(), title: await page.title() }, null, 2));
